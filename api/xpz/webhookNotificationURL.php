...
if(!empty($results)){
    ...
    $orderstatus=$results['status'];
        ...
        if($results['transaction']['type']== 'UCD'){
            ...
            $query = "UPDATE `gtech_payouts` SET  `orderremarks`='$pt_timestamp', `orderstatus`='$orderstatus', `status`='1', `payout_aar`='$payin_all' WHERE payout_request_id='$payin_request_id' ";
        }
